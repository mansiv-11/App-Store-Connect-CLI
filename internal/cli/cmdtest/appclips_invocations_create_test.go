...
func TestAppClipsInvocationsCreateSupportsInlineLocalization(t *testing.T) {
	client := newAppClipsTestClient(t, func(r *http.Request) {
...
func newAppClipsTestClient(t *testing.T, assertRequest func(*http.Request)) *asc.Client {
	t.Helper()
	...
	transport := appClipsRoundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(`{"data":{"type":"betaAppClipInvocations","id":"inv-1","attributes":{"url":"https://example.com/clip"}},"links":{}}`)),
			Request:    req,
		}, nil
	})
